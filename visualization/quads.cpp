...
void Quads::setModelViewMatrix(QMatrix4x4 &matrix)
{
    m_modelViewMatrix = matrix;
}

void Quads::generateVBOs()
{
    ensureInitialized();
    m_funcs->glGenBuffers(2, m_vboIds);
}

void Quads::ensureInitialized()
{
    if(!m_funcs) m_funcs = new QOpenGLFunctions(QOpenGLContext::currentContext());
}

QVector3D Quads::vectorFromColor(const QColor &color)
{
    return QVector3D(color.redF(), color.greenF(), color.blueF());
}

void Quads::update(const vector<QVector3D> &positions, const QVector3D &offset)
{
    ensureInitialized();
    ...
    for(int i=0; i<numPoints; i++) {
        ...
        QVector3D position(positions[i][0] + offset.x(), positions[i][1] + offset.y(), positions[i][2] + offset.z());
...
void Quads::createShaderProgram() {
    if (!m_program) {
...
void Quads::render(float lightFalloffDistance, const QMatrix4x4 &modelViewProjectionMatrix, const QMatrix4x4 &lightModelViewProjectionMatrix)
{
